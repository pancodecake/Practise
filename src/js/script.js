...
const rowProps = {
  ...
  date: "07.11.2022",
  svg: `<svg width="47" class='btns tableBtn' height="47" viewBox="0 0 47 47" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const row = ` <div class="table-row"><div class='table-row__div'><span id="name" >${
...
<ul class="menu">
    ...
    <li><a href="#" class="side-link tie "><img src="img/chain.svg" alt="img/icon-drag.svg">Привязать к категории</a></li>
 <li ><a href="#" class="side-link redact "><img src="img/Icon-key.svg" alt="img/icon-drag.svg">Редактировать пароль</a>
...
let cardPop = {
  tablePass: `         <div class=" cardSet side set">
    <ul class="menu">
        <li><div class="triangle"></div></li>
        <li><a href="#" class="side-link redact "><img src="img/Icon.png" alt="img/icon-drag.svg">Переместить</a></li>
        <li><a href="#" class="side-link dlt "><img src="img/chain.svg" alt="img/icon-drag.svg">Привязать к категории</a></li>
     <li ><a href="#" class="side-link move "><img src="img/Icon-key.svg" alt="img/icon-drag.svg">Редактировать пароль</a>
    ...
    <ul class="menu">
        ...
        ${
          document.title === "Projects"
            ? ""
            : `  <li class="menu__list"><a href="#" class="side-link move "><img src="img/Icon-a.svg" alt="img/icon-drag.svg">Переместить</a>
...
<div class="modall-chapter__content">
    <div class="modal-chapter__header">
        <div class="modal-add__title modal-add__title--bg">${
          document.title === "Projects"
...
function addingBtnMenuItems(that) {
  ...
  if (that.classList.contains("move")) {
    ...
    menuItemsToggle(menuMove);
  }
  else if (that.classList.contains("redact")) {
 
    menuItemsX(menuRedact);
  }
  else if (that.classList.contains("dlt")) {
 
    menuItemsX(menuDlt);
  }
  else if (that.classList.contains("tie")) {
    menuTie.style.transform = `translate(${cords.left - cords.width}px,${
        cords.top - cords.height
      }px`;
    menuItemsToggle(menuTie);
  }
  //---------------------------------------------------Functions
    ...
    cards.forEach((card) => {
      card.addEventListener("click", function (e) {
        if (e.target.classList.contains("cardSettings")) {
          e.target.classList.toggle("btnMenu-active");
          e.preventDefault();
          card.append(cardMain);
          const cardMenu = card.querySelector(".cardSet");

          console.log(e.target.classList);
          if (e.target.classList.contains("btnMenu-active")) {
            e.preventDefault();
          }

          console.log("facts");
          e.target.classList.contains("btnMenu-active")
          ...
          links.forEach((link) => {
            link.addEventListener("click", function (e) {
              if (this.classList.contains("redact")) {
                ...
                this.classList.toggle("btnMenu-active");
                body.append(cover);
                body.style.overflow = "hidden";
                if (this.classList.contains("btnMenu-active")) {
                ...
                this.classList.toggle("btnMenu-active");
                this.classList.contains("btnMenu-active")
                  ? (cardMove.style.visibility = "visible")
                  : (cardMove.style.visibility = "hidden");
              }
            });
          });
        }
